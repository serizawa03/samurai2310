<?php
    ...
    class booking_package_schedule {
        ...
        public function updateUser($administrator, $accountKey){
			...
			if ($currentUser === false || $currentUser->user_login !== sanitize_text_field($_POST['user_login'])) {
					...
					foreach ((array) $rows as $row) {
						...
						if ($i != 0) {
							...
							if ($time < 1440) {
								...
								return array("status" => "error", "message" => __("Schedule was not found", 'booking-package'), "sql" => $sql);
		...
		public function blockSameTimeBookingByUser($user_id, $calendarAccount, $startUnix, $endUnix, $emails) {
			...
			if (!is_null($user_id)) {
				...
				if (!is_null($endUnix)) {
					...
					$response['message'] = sprintf(__('You have already booked between %s and %s.', 'booking-package'), $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'), $this->dateFormat($dateFormat, $positionOfWeek, $endUnix, '', true, true, 'text'));
			...
			if (is_array($emails)) {
				...
				for ($i = 0; $i < count($emails); $i++) {
					...
					if (!is_null($endUnix)) {
						...
						$response['message'] = sprintf(__('You have already booked between %s and %s by %s.', 'booking-package'), $this->dateFormat($dateFormat, $positionOfWeek, $startUnix, '', true, true, 'text'), $this->dateFormat($dateFormat, $positionOfWeek, $endUnix, '', true, true, 'text'), $email);
					...
					if ($mode == "increase") {
						...
						foreach ((array) $rows as $row) {
							...
							if ($row['stop'] == 'false' && $remainder >= 0) {
								...
								$error = array('status' => 'error', 'error' => '9503', 'mode' => $mode, 'sql' => $sql, 'message' => __('There is a problem with the remaining capacity in the schedules.', 'booking-package'));
						...
						foreach ((array) $rows as $row) {
							...
							if (intval($row['capacity']) < $remainder) {
								...
								$error = array('status' => 'error', 'error' => '9503', 'mode' => $mode, 'sql' => $sql, 'message' => __('There is a problem with the remaining capacity in the schedules.', 'booking-package'), "data" => $row);
								throw new Exception(json_encode($error));
								#break;
		...
		public function getNotificationContents($calendarAccount, $customer, $email_id, $emailKey, $notificationContents, $emailFormat) {
			...
			if (empty($payName)) {
				...
				$payName = __('Pay locally', 'booking-package');
            ...
            foreach ((array) $notificationContents as $contentsKey => $contents) {
				...
				if (isset($_POST['receivedUrl'])) {
					...
					$contents = str_replace('[receivedUrl]', $_POST['receivedUrl'], $contents);
				...
				if ($payId == 'stripe') {
					...
					$payName = __("Pay with Credit Card", 'booking-package');
					
				} else if ($payId == 'stripe_konbini') {
					
					$payName = __("Pay at a convenience store", 'booking-package');
					
				} else if ($payId == 'paypal') {
					
					$payName = __("Pay with PayPal", 'booking-package');
			...
			foreach ((array) $sendEamilList as $emailKey => $target) {
				...
				foreach ((array) $emailData as $emailDataKey => $emailDataValue) {
					...
					if (!empty($coupon) && is_array($coupon) && isset($coupon['key'])) {
						...
						$emailDataValue = str_replace('[couponCode]', __('None', 'booking-package'), $emailDataValue);
					...
					if (!empty($coupon) && is_array($coupon) && isset($coupon['key'])) {
						...
						$emailDataValue = str_replace('[couponName]', __('None', 'booking-package'), $emailDataValue);
					...
					if (!empty($coupon) && is_array($coupon) && isset($coupon['key'])) {
						...
						$emailDataValue = str_replace('[couponDiscount]', __('None', 'booking-package'), $emailDataValue);
					...
					$optionsDetails = implode("\n", $optionsDetails);
					$emailDataValue = str_replace('[options]', $optionsDetails, $emailDataValue);
					...
					if (!is_null($services)) {
						...
						if (is_array($services)) {
							...
							$emailDataValue = str_replace('[services]', implode("\n", $detailsList), $emailDataValue);
							$emailDataValue = str_replace('[servicesExcludedGuests]', implode("\n", $detailsListExcludedGuests), $emailDataValue);
							$emailDataValue = str_replace('[servicesExcludedGuestsAndCosts]', implode("\n", $detailsListExcludedGuestsAndCosts), $emailDataValue);
							
						} else {
							
							$emailDataValue = str_replace('[service]', $service, $emailDataValue);
							$emailDataValue = str_replace('[services]', $service, $emailDataValue);
							$emailDataValue = str_replace('[servicesExcludedGuests]', implode("\n", $detailsListExcludedGuests), $emailDataValue);
							$emailDataValue = str_replace('[servicesExcludedGuestsAndCosts]', implode("\n", $detailsListExcludedGuestsAndCosts), $emailDataValue);
				...
				**/
				...
				if ($attachICalendarInEmail === 1) {
					...
					$ical = new booking_package_iCal($this->prefix, $this->pluginName);
